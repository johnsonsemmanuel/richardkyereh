    ...
    <>
        ...
        <div className="relative w-full max-w-3xl mx-auto px-6 lg:px-8 pb-16 lg:pb-20">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {article.tags.map((tag) => (
              <span
                key={tag}
                className="text-[10px] font-semibold tracking-[0.15em] uppercase text-muted-foreground border border-border px-3 py-1 rounded-full"
              >
                {tag}
              </span>
            ))}
      ...
      <article className="py-12 lg:py-16 bg-background">
        <div className="max-w-3xl mx-auto px-6 lg:px-8">
          ...
          <div className="mt-8 p-6 rounded-2xl bg-secondary border border-border shadow-card">
                  ...
                  {article.author.socialLinks && (
                    <div className="flex gap-2 ml-1">
                      {article.author.socialLinks.linkedin && (
                        <a href={article.author.socialLinks.linkedin} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors" aria-label="LinkedIn">
                      ...
                      {article.author.socialLinks.twitter && (
                        <a href={article.author.socialLinks.twitter} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors" aria-label="X (Twitter)">
                          <svg viewBox="0 0 24 24" fill="currentColor" className="size-3.5">
                      ...
                      {article.author.socialLinks.website && (
                        <a href={article.author.socialLinks.website} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors" aria-label="Website">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {related.map((r) => (
                <Link
                  ...
                  className="group block bg-secondary hover:bg-secondary transition-colors border border-border rounded-xl overflow-hidden shadow-card"
                >
                  ...
                  <div className="p-5">
                    <div className="flex items-center gap-2 text-[10px] text-muted-foreground uppercase tracking-[0.1em] font-semibold mb-2">
