    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
      ...
      {bookings.length === 0 ? (
        <div className="bg-card rounded-xl border border-input shadow-card p-8 text-center">
          <p className="text-sm text-foreground/50">No bookings found</p>
        </div>
      ) : (
        <>
          {/* Desktop table */}
              ...
              <div key={booking._id} className="bg-card rounded-xl border border-input shadow-card p-4">
                ...
                <div className="space-y-2 text-xs text-foreground/60">
                  <div className="flex items-center gap-2">
                    <span className="text-foreground/40 w-16 shrink-0">Email</span>
                    <span className="truncate">{booking.email}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-foreground/40 w-16 shrink-0">Service</span>
                    <span>{booking.service}</span>
                  </div>
                  {booking.date && (
                    <div className="flex items-center gap-2">
                      <span className="text-foreground/40 w-16 shrink-0">Date</span>
                      <span>{booking.date}{booking.time ? ` at ${booking.time}` : ""}</span>
                    </div>
                  )}
                  {booking.message && (
                    <div className="flex gap-2">
                      <span className="text-foreground/40 w-16 shrink-0">Message</span>
